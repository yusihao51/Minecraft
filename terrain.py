...
class TerrainGeneratorSimple(TerrainGeneratorBase):
    ...
    def generate_sector(self, sector):
        ...
        world = self.world
        x, y, z = sector
        from world import Sector, get_or_create
        sector_obj = get_or_create(Sector, x=x, y=y, z=z)
        if sector_obj in world.shown_sectors:
            for block in sector_obj.blocks:
                if block.blocktype not in self.autogenerated_blocks:
                    return
        ...
        if 0 <= by < (self.height_base + self.height_range):
            bytop = by + 8
            world_add_block, self_get_height, self_rand_choice = world.add_block, self.get_height, self.rand.choice #Localize for speed
            ...
            for x in xrange(bx, bx+8):
                for z in xrange(bz, bz+8):
                        ...
                        if y <= self.water_level:
                            if G.TERRAIN_CHOICE != "desert":# was y == self.height_base -- you can have water!
                                if G.TERRAIN_CHOICE == "snow":  # top block is ice
                                    world_add_block((x, self.water_level, z), ice_block)
                                else:
                                    world_add_block((x, self.water_level, z), water_block)
                                # world_add_block((x, y -1, z), water_block)
                                world_add_block((x, self.water_level -2, z), self_rand_choice(self.underwater_blocks))
                                world_add_block((x, self.water_level -3, z), dirt_block)
                            if G.TERRAIN_CHOICE == "desert":  #  no water for you!
                                world_add_block((x, y +1, z), sand_block)
                                world_add_block((x, y, z), sand_block)
                                world_add_block((x, y -1, z), sand_block)
                                world_add_block((x, y -2, z), sandstone_block)
                                world_add_block((x, y -3, z), sandstone_block)
                            y -= 3
                        elif y < bytop:
                            if G.TERRAIN_CHOICE == "island":#  always sand by the water, grass above
                                if y > self.island_shore:
                                    ...
                                    mainblock = sand_block
                            world_add_block((x, y, z), mainblock)  # grass_block)
                            ...
                            if mainblock == grass_block or mainblock == snowgrass_block:
                                world_add_block((x, y -1, z), dirt_block)
                                world_add_block((x, y -2, z), dirt_block)
                                world_add_block((x, y -3, z), dirt_block)
                            elif mainblock == sand_block:
                                world_add_block((x, y -1, z), sand_block)
                                world_add_block((x, y -2, z), sand_block)
                                world_add_block((x, y -3, z), sandstone_block)
                            elif mainblock == stone_block:
                                world_add_block((x, y -1, z), stone_block)
                                world_add_block((x, y -2, z), stone_block)
                                world_add_block((x, y -3, z), stone_block)
                    ...
                    for yy in xrange(by, y):
                        ...
                        world_add_block((x, yy, z), self_rand_choice(blockset))
                    if by == 0:
                        world_add_block((x, 0, z), bed_block)
